...
import { defaultQuizPayload } from '@/utils/utils';

export default function HomePage() {
  const router = useRouter();

  const handleDefaultQuizClick = async () => {
    const link = await fetch('/api/generate-quiz', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(defaultQuizPayload),
    }).then(res => res.json()).then(data => data.quizLink);
    router.push(`${window.location.origin}/quiz/${encodeURIComponent(link)}`);
  }
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="max-w-2xl w-full text-center bg-white p-8 rounded-lg shadow-md">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-center">
          ...
          <button
            onClick={handleDefaultQuizClick}
